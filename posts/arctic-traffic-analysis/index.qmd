...
```{r}
mod <- lm(Value ~ abs(Extent) + Year + Season, data = anomaly_north)
mod %>%
...
$$
```{r}
mod <- lm(Value ~ Season + Year + Region + Extent, data = all_anomaly_dat)
mod %>%
  tbl_regression(intercept= TRUE) %>%
  as_gt() %>%
  gt::tab_source_note(gt::md("*Kelly Kapsar, Benjamin Sullender, and Aaron Poe. 2022. North Pacific and Arctic Marine Vessel Traffic Dataset (2015-2020); Hexagon Data.*"))
...
$$
```{r}
mod <- lm(Value ~ Region +Season + Region:Season, data = all_anomaly_dat)
...
```{r}
mod <- lm(Value ~ Extent + Year + Region +Season + Region:Season, data = all_anomaly_dat)
